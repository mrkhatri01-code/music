        ...
        <form action="{{ route('admin.movies.update', $movie->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group" style="margin-bottom: 1.5rem;">
                ...
                <input type="text" id="name" name="name" class="form-control" value="{{ old('name', $movie->name) }}"
                    required placeholder="Enter movie name"
                ...
                <div class="form-group">
                    ...
                    @if($movie->cover_image)
                        <div style="margin-bottom: 0.5rem;">
                            <img src="{{ asset($movie->cover_image) }}" alt="Current Cover"
                                style="max-height: 150px; border-radius: 8px; border: 1px solid #cbd5e0;">
                        </div>
                    @endif
                    <input type="file" id="cover_image" name="cover_image" class="form-control" accept="image/*"
                        style="width: 100%; padding: 0.75rem; border: 1px solid #e2e8f0; border-radius: 8px;">
                    <small style="color: #718096;">Leave empty to keep current image</small>
                    @error('cover_image')
                        <span style="color: #e53e3e; font-size: 0.875rem;">{{ $message }}</span>
                    @enderror
                </div>
            ...
            <div class="form-group" style="margin-bottom: 1.5rem;">
                ...
                <textarea id="description" name="description" class="form-control" rows="4"
                    ...
                    style="width: 100%; padding: 0.75rem; border: 1px solid #e2e8f0; border-radius: 8px;">{{ old('description', $movie->description) }}</textarea>
                @error('description')
                    <span style="color: #e53e3e; font-size: 0.875rem;">{{ $message }}</span>
                @enderror
            </div>
