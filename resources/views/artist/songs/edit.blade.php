...
@section('content')
        ...
        <div class="card-body">
            <form action="{{ route('artist.songs.update', $song->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        <!-- Artist is fixed to current user -->
                         <div class="form-group">
                            <label>Artist</label>
                             <input type="text" class="form-control" value="{{ auth()->user()->artist->name_english }}" disabled style="background-color: #e9ecef;">
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // --- 2. Nepali -> Roman (Backend API) ---
            async function romanize(text) {
                if (!text || !text.trim()) return null;
                try {
                    const response = await fetch('{{ route('artist.romanize') }}', {
                        method: 'POST',
                        headers: {
            ...
            // Flag to prevent infinite loops (A update B, B update A...)
            ...
            // --- Lyrics Logic ---
            if (lyricsRomanized && lyricsUnicode) {
                let timeoutRoman = null;
                let timeoutUnicode = null;

                // 1. Romanized -> Unicode
                lyricsRomanized.addEventListener('input', function() {
                    if (isUpdating) return;
                    if (timeoutRoman) clearTimeout(timeoutRoman);
                    
                    const text = this.value;
                    timeoutRoman = setTimeout(async () => {
                        if (!text.trim()) return;
                        isUpdating = true;
                        const trans = await transliterate(text);
                        if (trans) {
                             lyricsUnicode.value = trans;
                        }
                        isUpdating = false;
                ...
                // 2. Unicode -> Romanized
                lyricsUnicode.addEventListener('input', function() {
                    if (isUpdating) return;
                    if (timeoutUnicode) clearTimeout(timeoutUnicode);
                    
                    const text = this.value;
                    timeoutUnicode = setTimeout(async () => {
                        if (!text.trim()) return;
                        isUpdating = true;
                        const rom = await romanize(text);
                        if (rom) {
                            lyricsRomanized.value = rom;
                        }
                        isUpdating = false;
                    }, 1000); // 1s debounce
                });
